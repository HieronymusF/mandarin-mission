...
1. 许可清晰且通过普通话人工试听的课程音频：三条 CosyVoice I1/L2/I3 已通过整组试听、资源校验、APK 打包和 Sony 真机逐条播放；
2. 本地录音、回放与明确权限流程：Sony 真机已通过；
3. 音频/语音服务不可用时不阻断课程：自动测试与无录音降级通过，服务不可用真机制造未完成；
4. 三种缺失步骤已进入 11 步课程并通过自动验证、Android 模拟器与 Sony 真机；咖啡场景图已作为 `ready` 资产接入，第二版构图和中文+拼音选项已获用户真机确认；
5. 真实 Android 设备完成课程 → 杀进程 → 进度保留 → 正确时间出现复习：Sony 真机已通过；
6. 飞行模式完成已下载课程：Sony 真机已通过。

不要因为代码容易写而提前扩展地图、社交、付费或 AI 对话。

## 10. 当前最高优先级：M1 收尾与平台边界

到期复习与音频/录音核心架构已完成。`image_choice`、`tone_contrast`、`order_tokens` 已实现并接入 11 步 Fixture；自动测试、Android 模拟器和 Sony 真机布局/交互已通过，完整完成页、6 项到期复习与两次冷启动持久化也已复验。咖啡场景图和 CosyVoice 三条音频已通过内容与真机放行，Fixture 已升为 `release`。
...
- 三条 CosyVoice WAV 已通过 `CosyVoice-300M-SFT` 内置 `中文女`、seed 1986 本地生成，并以 Apache-2.0 模型/工具来源接入 `assets/audio/cosyvoice/` ready 资产；文件为 22.05 kHz/16-bit/mono，仅由 float WAV 转换为 PCM16，没有真人克隆或声学后处理。
- 新 APK 已在 Sony 真实课程入口逐条播放“我要”、“咖啡”和“我要一杯咖啡”，Android 日志确认音轨正常结束并释放焦点；这是新资产本身的证据，不沿用旧音频结论。
...
- 真实来电中断与恢复；当前 Sony 无 SIM，不能用 Home 或 ADB 模拟冒充来电证据；
- 如果后续更换 TTS Provider、声线、参数或音频文件，必须重新验证许可、哈希、人工听感、APK 打包和真机播放；
- 音频/录音不可用时保留可完成的降级路径；
