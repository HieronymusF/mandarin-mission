...
- 生产启动已接首次使用门禁，路由已有 Journey/Review/Settings 主导航且课程保持独立；帮助、隐私、条款状态页、本地数据清理、版本信息、偏好/服务状态和本地通知/诊断选择已实现，但仍无真实客服/法律资源、通知投递与诊断服务、个人/账号或完整设置；
- 客户端没有远端认证、同步或商店支付接线；
...
1. 继续完成 App 信任外壳：首次引导、主导航、设置、帮助/隐私/条款状态、数据管理、版本信息、偏好/服务状态和本地通知/诊断选择已交付；下一步补真实通知投递与诊断服务、真实客服/法律资源与真机验收；
