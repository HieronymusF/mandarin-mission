...
> 最近核对基线：`main` / `92f748b`（2026-07-27）；本轮再次核对 App 路由、Feature、客户端依赖、Android manifest 与 Go handler，并区分 M2 内容完成和公开 MVP 产品完成。
...
| App 外壳、客服与法律 | ❌ 未实现 | 生产路由只有 Journey、课程、复习；无引导、主导航、设置、客服、隐私、条款或数据管理 |
...
| 12 | 设置、客服、隐私/条款和数据管理 | ❌ | 路由和 Feature 均不存在 |
...
### P0 — 阻塞里程碑
1. **设置、客服、隐私/条款与数据控制缺失**——这是公开产品的信任外壳，不是商店表单可以替代的材料。
