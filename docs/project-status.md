...
> 最近核对基线：`agent/app-settings-trust-center` / `39e442a` + 当前工作树（2026-07-27）；本轮实现并验证设置与信任中心首切片，同时保留 M2 内容完成与公开 MVP 产品完成的边界。
...
| App 外壳、客服与法律 | ⚠️ 首切片完成 | 已有 Journey/Review/Settings 主导航、帮助/隐私/条款状态页、本地数据清理和版本信息；引导、真实支持/法律资源及完整设置仍缺 |
...
| 12 | 设置、客服、隐私/条款和数据管理 | ⚠️ 首切片完成 | 生产路由与 `features/settings/` 已落地；无假链接，本地数据清理有二次确认；真实支持/法律资源和完整客服仍缺 |
...
### P0 — 阻塞里程碑
1. **信任外壳仍未闭环**——首切片已落地主导航、设置状态页和本地数据清理；首次引导、真实客服/法律资源、完整偏好与真机验收仍是公开产品门禁。
