...
| 12 | App 外壳、设置、客服、隐私/条款和数据管理 | ⚠️ 三个切片完成 | `features/onboarding/` 与 `features/settings/` 已进入生产路径；偏好页如实标出未接入服务，无假链接，本地数据清理有二次确认；真实通知/选择控制、支持/法律资源和完整客服仍缺 |
...
### P0 — 阻塞里程碑
1. **信任外壳仍未闭环**——首次引导、主导航、偏好/服务状态页、信任状态页和本地数据清理已落地；真实通知与可选收集控制、客服/法律资源、完整偏好与新外壳真机验收仍是公开产品门禁。
