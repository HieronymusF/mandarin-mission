...
| 公开 MVP | 6 个地点、约 30 节课 | 设置与客服、隐私与数据控制、账号同步、订阅、基础语音、分析、崩溃监控 | [公开 MVP App 完成门禁](requirements/public-mvp-completion.md)全部通过，且核心指标达到第 15 节定义的 Go 线 |
...
- 以[公开 MVP App 完成门禁](requirements/public-mvp-completion.md)为唯一放行清单；先完成 App 产品，不开始正式签名、商店素材或审核提交。
- 完成首次使用、主导航、设置、帮助与客服、隐私政策、服务条款、数据选择与账号删除入口。
- 扩展到 6 个地点、约 30 节课。
...
验收：跨设备进度可合并；购买与恢复购买可验证；客服与法律入口真实可用；账号删除能够清除云端个人数据；G1—G6 全部通过。商店审核材料属于下一阶段，不再作为 App 功能完成的替代证据。

### 阶段 5：发布与观察（第 19—24 周）

- 只有阶段 4 的 G1—G6 全部通过，才配置正式签名、制作商店资料并提交审核。
...
- [x] 经审核的课程音频、本地录音、回放和服务不可用降级；
- [x] 正式插画/音频资产和内容发布校验，以及 10 种步骤类型的自动与真实 Android 设备验收；
- [x] 一台真实 Android 设备上的课程 → 重启 → 到期复习闭环。
- [x] M2 扩展到 3 个地点、12 节课和 3 个地点终局，并成为默认正式内容包；
- [ ] 按[公开 MVP App 完成门禁](requirements/public-mvp-completion.md)补齐设置/客服/法律、账号/同步/删除、订阅、游戏化、分析/崩溃和公开 MVP 内容；
- [ ] G1—G6 全部通过后再开始商店发布工作。

M1 和 M2 的本地学习里程碑均已完成；它们只证明内容与学习闭环，不代表公开 MVP 已完成。当前阶段以公开 MVP App 完成门禁为准，上架不是当前里程碑。
...
公开 MVP 可以进入商店发布阶段，必须先完整通过[公开 MVP App 完成门禁](requirements/public-mvp-completion.md)的 G1—G6，并同时满足：

- 6 个地点和约 30 节课全部通过内容、语言和音频审核；
- 设置、客服、隐私政策、服务条款和数据控制均为真实可用的生产功能；
