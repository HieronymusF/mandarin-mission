...
Mandarin Mission 采用“移动端本地优先 + 托管云服务 + 数据驱动课程”的实现路线。
...
| 能力 | 选择 | 使用边界 |
|---|---|---|
| 登录与用户 | Supabase Auth | 原型允许匿名使用；购买或跨设备同步前再要求登录 |
| 云数据库 | Supabase Postgres | 保存用户档案、进度快照、同步事件、内容版本和远端配置 |
| 文件存储 | Supabase Storage | 保存可下载课程包、音频和插画；首个 12 课也可随 App 内置 |
| 轻量服务端 | Supabase Edge Functions | 语音服务代理、订阅 Webhook、内容签名与需要密钥的操作 |
| 订阅 | RevenueCat | 统一处理 App Store/Google Play 购买状态，不自建票据校验 |
| 发音评测 | Azure Speech，置于 Provider 接口后 | 只评测已知目标文本；密钥仅在服务端；可通过 Feature Flag 关闭 |
| 分析 | Firebase Analytics | 只记录产品事件和非敏感分群，不上传录音或用户自由文本 |
| 崩溃 | Firebase Crashlytics | 发布前必须验证测试崩溃和符号上传 |

Supabase Flutter SDK 可覆盖数据库、用户管理、文件和 Edge Functions；所有用户表必须启用 Row Level Security。[Supabase Flutter](https://supabase.com/docs/reference/dart/introduction) [Supabase Auth](https://supabase.com/docs/guides/auth)

RevenueCat Flutter SDK 封装 Apple/Google 的购买流程并用 entitlement 表示订阅权限，适合独立开发者避免自建支付后台。[RevenueCat Flutter](https://www.revenuecat.com/docs/getting-started/installation/flutter)
...
独立开发阶段的主要风险是课程与学习闭环，而不是数据库框架。自建认证、支付校验、对象存储和运维会增加无法形成产品差异的工作量。只有在托管服务成本、地区合规或数据规模出现明确问题后，才评估迁移。
...
5. 订阅权限以 RevenueCat entitlement 为准，本地只缓存最近状态。
...
│  └─ fixtures/
├─ packages/
│  └─ content_validator/         # 内容 Schema、引用和资源完整性校验
├─ supabase/
│  ├─ migrations/
│  ├─ functions/
│  └─ seed.sql
...
第一阶段不建立 monorepo 管理框架；普通目录和脚本足够。只有共享包数量明显增加时再引入额外工作区工具。
...
| `KnowledgeItem` | `id`, `kind`, `hanzi`, `pinyin`, `english`, `audio`, `tags` | 词、短语、句型或汉字 |
...
4. 校验音频采样、时长、响度和资源哈希；记录授权来源。
5. 用内部预览页逐课检查图片、文字、音频和答案。
6. 生成带版本号的 Manifest，并在测试环境发布。
7. 至少经过英语母语编辑、普通话录音检查和国际中文教师审核后进入正式内容包。
...
- `teach_card`：场景图、真人音频、汉字、拼音和英文解释；
- `image_choice`：意义识别；
- `listen_choice`：听音识别；
- `tone_contrast`：声调对比；
- `order_tokens`：短句排序；
...
- `entitlement_audit`：订阅 Webhook 处理结果。

每个用户数据表启用 RLS，用户只能读写自己的行。管理操作只允许服务端角色执行，服务端密钥不得进入 App 或仓库。
...
- 设置项使用最后写入胜出；订阅权限始终以 RevenueCat 最新状态为准。
...
- Firebase Crashlytics 上线前要完成一次可控测试崩溃，确认堆栈可读。[Flutter Crashlytics](https://firebase.google.com/docs/crashlytics/flutter/get-started)
...
- 扩展到 6 个地点、约 30 节课。
- 增加 Supabase 登录、Outbox 同步和内容增量发布。
- 接入 RevenueCat entitlement、恢复购买和付费墙。
...
- 合并检查：格式、静态分析、单元/Widget 测试、内容校验、Secret 扫描。
...
- 客户端只使用 Supabase publishable key 和各平台公开 SDK key；服务角色密钥、Azure 密钥和 RevenueCat secret 仅在服务端。
- Supabase 表默认启用 RLS，并为每个策略编写最小权限测试。
...
按以下顺序创建首批 GitHub Issues：

1. `chore: scaffold Flutter mobile app`
2. `ci: add format analyze test and content checks`
3. `feat(content): define course schema and validator`
4. `feat(data): add Drift schema and migration tests`
5. `feat(journey): build single-location map shell`
6. `feat(lesson): render data-driven lesson steps`
7. `feat(progress): persist attempts and mastery states`
8. `feat(review): implement v1 box scheduler`
9. `feat(audio): add playback recording and local fallback`
10. `content(cafe): ship first end-to-end lesson`

第一里程碑完成定义：上述 10 项关闭，并在一台真实 Android 设备上从地图完成“点咖啡”课程、重启 App、看到正确的到期复习。
...
| 包名/Bundle ID | 暂不写死 | Flutter 工程创建前 |
| GitHub 可见性 | 私有仓库 | 创建远端仓库时 |
| 开源协议 | 暂不授权 | 仓库转公开前 |
| 云服务地区 | 依据首批测试者地区选择 | 建 Supabase/Azure 项目前 |
...
- 没有客户端 Secret，RLS 和账号删除经过验证；
- Crashlytics、核心分析事件、语音失败率和订阅状态可观测；
