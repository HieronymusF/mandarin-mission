...
M1 完成定义：上述剩余项关闭，并在飞行模式下完成已下载的“点咖啡”课程；杀掉 App 后进度存在；正确时间出现到期复习；没有云服务也不阻断核心课程。2026-07-23 Sony `XQ-DQ72` 已通过飞行模式完整课程、杀进程后的完成进度保留和 8 项必做到期复习保存；M1 仍受经审核音频、缺失步骤类型与正式资产阻塞。
