...
> 任务等级：M
> 当前清单：App `0.2.0+3` / inventory revision 2
> 最近核对：2026-07-27
...
- 没有远端推送或诊断服务商，也没有新增网络传输；
- Android 本地每日提醒只在用户选择时间后请求通知权限并在设备上调度；诊断选择仍只是未来服务许可；
...
| 本地提醒与可选诊断 | `preferences.notifications.enabled.v1`、`preferences.notifications.reminder_minutes.v1`、`preferences.diagnostics.enabled.v1` | Shared Preferences；清除学习数据时保留，可随时撤回 | 提醒仅在 Android 本机调度；不注册 Token、不联网，诊断不发送 | App preferences 选择时间、开/关 | `features/settings/data/app_preferences_store.dart`、`features/settings/data/local_learning_reminder_service.dart` |
...
- Android 主 Manifest 显式声明 `RECORD_AUDIO` 与用于重启恢复调度的 `RECEIVE_BOOT_COMPLETED`；通知插件合并运行时通知权限，用户确认提醒时间后才请求。当前代码没有精确闹钟权限、推送 Token、账号、广告 ID、商店支付或定位权限；
- iOS `Info.plist` 当前未形成公开 MVP 的麦克风隐私描述与系统验收，不能从 Android 结果推导 iOS 已通过；
- 与数据处理相关的运行依赖包括本地音频/录音、SQLite/Drift、Shared Preferences、本地通知/时区、权限、包信息和外部链接能力；没有 Firebase、Sentry、远端分析、推送、身份或购买 SDK；
...
4. 当前实现使用一个稳定提醒 ID 和设备本地时区，以 `inexactAllowWhileIdle` 每日调度，不申请精确闹钟权限；重启后由插件接收器恢复。
5. 撤回时取消稳定 ID 对应的已安排提醒；本机选择成功写回后 UI 才显示 Off。调度或偏好存储失败时不得虚报成功。
6. 任何远端推送、设备 Token 或营销消息属于新范围，必须另做数据清单、服务端删除和同意门禁。

当前最小 Adapter 已实现：读取平台可用性/权限状态、在用户动作后请求权限、按稳定提醒 ID 安排或取消本地提醒，以及打开系统设置。它不是远端通知服务，也不证明设备已实际在指定时间展示通知。
...
- 不接远端推送、分析或崩溃 SDK；
- 不生成通知 Token，不发送测试事件；Android 通知权限仅由用户确认本地提醒时间后请求；
- 不发布隐私政策、服务条款或商店 Data safety/App Privacy 答案；
- 不实现账号、同步、删除账号、订阅或服务器数据保留；
- 不改变 Drift schema 或现有清除学习数据语义；新增提醒分钟数键由清除学习数据保留。
