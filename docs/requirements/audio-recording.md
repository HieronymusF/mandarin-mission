...
用户在课程中需要听到普通话发音并进行口语练习。当前开发包已接入三条由 Apache-2.0 `CosyVoice-300M-SFT` 内置 `中文女` 本地生成的音频，已通过整组普通话人工试听、资源校验、APK 打包和 Sony 真机逐条播放。录音与本地回放功能继续按真实设备验收。
...
1. [x] CosyVoice 替代整组通过普通话人工试听，接入后可正常播放、暂停、重新播放
