...
| Pass 2 | 内容校验反例、调度边界、重复完成、媒体状态与测试、CI、Go handler、全仓一方源码目录 | 内容引用、稳定 ID、对话可达性和 release 资产状态有校验；调度要求 UTC 并限制箱位与同日补救；媒体服务有本地降级，但真人资产和真机验收未完成；CI 分为 mobile、learning-core、api 三个 job | 文档必须把音频标为部分完成，把 `services/api` 标为独立骨架，并明确生成文件、研究资料与当前主流程的边界 |
...
| 输入 | 包内 [点咖啡课程数据](../../content/fixtures/cafe-course.json)；当前为 `schemaVersion: 1`、`status: draft`、版本 `0.1.0` |
...
| 边界或失败 | 无效内容在建模前被拒绝；保存失败保留当前步骤并提供重试；非 UTC 调度输入抛错；`planned` 音频资产返回不可用状态而非伪造播放 |
...
| --- | --- | --- | --- | --- |
| App 从 Journey 进入 `cafe-01`，课程结束后返回 Journey | [路由定义](../../apps/mobile/lib/app/router.dart)、[Journey 入口](../../apps/mobile/lib/features/journey/presentation/journey_page.dart)、[课程页面](../../apps/mobile/lib/features/lesson/presentation/lesson_overview_page.dart) | 已确认 | Journey 目前仍是单地点外壳 | walkthrough、code map |
...
| 当前 Fixture 有 1 地点、3 知识点、1 课程、8 步、1 对话和 4 资产 | [点咖啡课程数据](../../content/fixtures/cafe-course.json) | 已确认 | 数据包是 `draft`；3 个音频资产均为 `planned` | README、walkthrough、内容模块 |
...
| `ready` 音频路径必须由 `audioAssetId` 解析，`planned` 或缺失资源进入降级 | [内容模型](../../apps/mobile/lib/data/content/course_content_models.dart)、[内容 Repository 测试](../../apps/mobile/test/data/content/course_content_repository_test.dart) | 已确认 | 真人普通话资源尚未加入 | 媒体模块 |
| 媒体 UI 区分播放状态、资产降级、权限分支、服务不可用、录音和回放 | [媒体组件测试](../../apps/mobile/test/shared/presentation/media_controls_test.dart)、[媒体 Controller 测试](../../apps/mobile/test/data/audio/audio_controller_test.dart) | 已确认 | 仍缺 Android 真机和生命周期证据 | 媒体模块、quality review |
...
4. 真人音频变为 `ready` 并通过 Android 真机播放、权限、录音和回放验收。
