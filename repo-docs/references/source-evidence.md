...
| 输入 | 包内 [点咖啡课程数据](../../content/fixtures/cafe-course.json)；当前为 `schemaVersion: 1`、`status: release`、版本 `0.1.7` |
