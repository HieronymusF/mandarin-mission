...
| 通知与分析/崩溃选择默认关闭，可在本机持久化和撤回；服务未接入时不会请求通知权限或发送数据 | [偏好状态](../../apps/mobile/lib/features/settings/application/app_preferences_providers.dart)、[偏好存储](../../apps/mobile/lib/features/settings/data/app_preferences_store.dart)、[偏好页面](../../apps/mobile/lib/features/settings/presentation/app_preferences_page.dart)、[设置测试](../../apps/mobile/test/features/settings/settings_page_test.dart) | 已确认 | Android 模拟器已验证 44×44dp 触控语义、开启/撤回和跨进程恢复；这不是通知投递、诊断 SDK 或 Sony 真机验收 | README、代码地图、公开 MVP 完成门禁 |
