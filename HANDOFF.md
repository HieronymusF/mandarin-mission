...
> 最后更新：2026-07-27 14:30（Asia/Hong_Kong）
> 项目：`D:\mandarin-mission\mandarin-mission`
> Git：当前本地分支 `agent/app-preferences-status`，HEAD 与 `origin/main` 均为 PR [#24](https://github.com/HieronymusF/mandarin-mission/pull/24) 的 merge commit `01c9580`；该提交的独立 `main` CI run [`30240937682`](https://github.com/HieronymusF/mandarin-mission/actions/runs/30240937682) 已通过 `mobile`、`learning-core`、`api`。首次引导功能提交为 `0b20380`；内容发布提交与 tag `v0.2.0` 均为 `07154a1`。当前分支、提交、PR、CI 和合并状态须从 Git/GitHub 实时核验，不从本行推断
...
- 无代码构建阻塞。PR #24 已合并，merge commit `01c9580` 的独立 `main` CI 三项全绿；当前 `agent/app-preferences-status` 切片本地验证已通过但尚未提交、推送或建立 PR，因此没有本切片的远端 CI。
...
1. **先收口 App preferences 分支，再继续关闭 G1/G2 信任外壳**：用户授权后按明确文件范围提交、推送并走 PR/CI；随后补真实通知与可选收集控制、可收件支持渠道、正式隐私政策/服务条款和客服流程。产品资源未提供时保持未配置状态，不造假值；补 Sony 真机视觉时必须和模拟器证据分开记录，不能覆盖或清除正式学习数据。
