...
工程基线、代码优先 UI 基线、课程播放器、进度持久化和复习数据层已经建立。除非用户另有要求，接下来按以下顺序：

1. 实现本地到期复习 UI 和 Journey 入口；
2. 实现音频播放、录音、回放和本地降级；
3. 补齐“点咖啡”缺失的首批步骤类型与真实资产；
4. 在真实 Android 设备验证课程 → 重启 → 到期复习闭环；
5. 扩展到 3 个地点、12 节课和可玩循环。
