...
- 当前阶段：一期工程基础已建立；包含纯 Dart 学习核心、课程 Schema/Fixture、Go API 容器、Flutter Android/iOS App 外壳，以及 Drift v1 本地 schema 和移动端 CI。
- 当前下一里程碑：完成“点咖啡”一节课的 Android 端到端垂直切片。
- 当前下一小模块：建立课程内容 Repository/加载层，以版本化 Fixture 驱动课程读取；暂不修改 UI。
