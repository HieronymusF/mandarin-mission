...
- 开发协作分工：UI 在 Figma 上设计、画原型、确认由具备 Figma 插件能力的 AI 代理（如 codex、Claude）主导完成，人类开发者只做辅助改动；当前接入的代理（ZCode/GLM）没有 Figma 画图能力，只负责读取已确认的 Figma 设计稿并实现为代码。所有 AI 代理都可参与其余代码实现（Flutter、Go、Dart 核心、Drift、CI、内容校验、测试等）。需要实现 UI 时，以已确认的 Figma 节点链接或截图为准；代理不主动在 Figma 之外另起替代性 HTML mockup 或独立视觉原型。
- 当前阶段：一期工程基础已建立；包含纯 Dart 学习核心、课程 Schema/Fixture、Go API 容器、Flutter Android/iOS App 外壳、Drift v1 本地 schema、安装包内课程 Repository、由内容数据驱动的七步“点咖啡”课程播放器、课程练习与四维掌握度持久化，以及按到期时间和薄弱维度生成的本地复习队列数据层。
- 当前下一里程碑：完成“点咖啡”一节课的 Android 端到端垂直切片。
- 当前下一小模块：先在 Figma 确认复习入口、四维题型与忘记/模糊/记得反馈流程，再实现本地到期复习 UI。
...
- 每个功能从立项到交付按 `docs/development-workflow.md` 的七阶段推进：需求文档 → Figma 设计规范 → Figma 原型 + 后端（并行）→ 前端还原 → 联调 → 测试 → 交付。
- 每个阶段都有明确产物和门槛，未达标不得进入下一步。
- 纯本地、无 UI 的特性（算法、数据层、CI）可跳过设计相关阶段，但需求文档和测试仍然适用。
- 遇到本节“前端任务的 Figma 前置流程”与本流程冲突时，以本流程为准；前者作为阶段 3a/4 的细节补充保留。
