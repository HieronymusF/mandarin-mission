...
- 当前阶段：一期工程基础已建立；包含纯 Dart 学习核心、课程 Schema/Fixture、Go API 容器、Flutter Android/iOS App 外壳、Drift v1 本地 schema、安装包内课程 Repository、由内容数据驱动的七步“点咖啡”课程播放器，以及课程练习、四维掌握度、完成进度与同步 Outbox 的本地事务持久化。
- 当前下一里程碑：完成“点咖啡”一节课的 Android 端到端垂直切片。
- 当前下一小模块：基于已持久化的 `MasteryState.dueAt` 建立本地到期复习队列，并把复习提交接入现有 v1 分箱调度。
