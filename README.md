...
> 当前状态：M1 本地学习闭环已在 Sony Android 15 真机通过，覆盖 11 步课程、包内音频、本地录音/回放、权限分支、汉字书写、课程完成状态、冷启动进度保留、到期复习和飞行模式课程。咖啡场景图与中文+拼音选项已获用户真机确认；三条 Apache-2.0 CosyVoice 课程音频通过整组人工试听、资源校验、APK 打包和 Sony 真机逐条播放，内容包已升为 `release`。真实来电中断因验收设备无 SIM 未验证。
