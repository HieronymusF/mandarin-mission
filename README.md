...
> 当前状态：M2 `0.2.7` 已成为 App 默认正式课程包，覆盖 Café、Market、Metro 3 个地点、12 节课、3 个地点终局、52 个知识点和 53 个 `ready` 资产。新增范围 14/14 单元、49 条新增普通话音频、线性解锁、本地进度与复习恢复均已有 Sony Android 15 真机证据；本轮正式 Release 构建与完整自动门禁已通过。真实来电中断因验收设备无 SIM 未验证，商店签名仍未配置。
...
| 公开 MVP | 6 个地点、约 30 节课 | 设置与客服、隐私与数据控制、账号与同步、订阅、基础语音、分析、连胜、印章册 |
...
- 语音：包内三条课程音频使用 Apache-2.0 `CosyVoice-300M-SFT` 内置 `中文女` 生成，已记录声线、参数、合成文本差异和 SHA-256，并通过整组人工试听与 Sony 真机播放。Azure Speech 发音评测属于后续服务端能力，尚未接入当前 App 主路径。
...
- 从内容元数据解析并播放三条已放行的 CosyVoice 包内音频，课程与 listening 复习共用同一资产路径；
...
- Journey 从本地数据库读取 12 节课和 3 个地点终局的真实完成状态，并据此控制逐课、终局和跨地点解锁；
...
- [x] 完成 11 步“点咖啡”课程的真实 Android 设备复验与冷启动进度保留。
- [x] 接入咖啡场景图片，并把图片题选项改为中文+拼音；第二版构图已获用户真机确认。
- [x] 完成正式内容放行：CosyVoice 整组试听、APK 打包与 Sony 真机逐条播放通过。
- [x] 扩展到 3 个地点、12 节课和 3 个地点终局，完成人工逐页 UX，并切为默认正式内容包。
- [x] 完成设置与信任中心首切片：Journey/Review/Settings 主导航、帮助/隐私/条款状态页、本地数据清理、版本信息及失败/离线/200% 字号测试；生产支持与法律资源仍待真实值。
- [x] 完成首次使用引导首切片：首次启动先说明学习方式、离线与本地数据边界，完成后持久化进入 Journey，并可从 Settings 重看；Sony 真机视觉仍待补。
- [x] 完成 App 偏好与本地选择切片：如实展示语言、离线音频、可选麦克风和未接入服务；通知与分析/崩溃选择默认关闭，可在本机持久化和撤回。真实通知投递、诊断服务接入与 Sony 真机视觉仍待补。
- [ ] 根据激活、D1/D7 留存和 7 日回忆率决定是否扩展公开 MVP。
- [ ] 按[公开 MVP App 完成门禁](docs/requirements/public-mvp-completion.md)继续补齐信任外壳、账号与同步、订阅、基础语音、分析、连胜、印章册，以及 6 个地点、约 30 节课。
- [ ] 仅在全部 App 内门禁和封闭测试通过后，开始正式签名、商店素材与审核提交。
